    ...
    $("#gotrav").click(function() {
        ...
        if (type == 'con') {
            $.ajax({
                type: "POST",
                data: filter2json(),
                url: "../apps/ItemFilterTrav.php?type=" + type + "&num=" + num,
                beforeSend: function() {
                ...
                success: function(d) {
                    if (d == 'Fail' || d == 'NoLogin' || d == 'ConnectFail') {
                        ...
                        var thep = eval("(" + d + ")");
                        var col = ["#e2fbff", "#ffffff"];
                        if (parseInt(thep.length) == 0) {
                            $("#showres").html("<div id=\"noret\">" + nofilterret + "</div>");
                            $("#pages").html("");
                            return;
                        }
                        var str_show_key = "<div class='key' style='font-size:large;color:red'>Result " + thep.length +"</div>";
                        for(var i = 0; i < thep.length; ++i){
                            str_show_key += "<label></label><div class=\"key\" style=\"font-size: medium;background:" + col[i % col.length] +  "\";>" + thep[i] + "</div>";
                        }
                        $("#showres").html(str_show_key);
                    }
                }
            });
        }
...
function filter2json() {
    var regex = $("#keyfilterin").val();
    var jsonstr = "{'data':'" + regex + "'}";
    var p = eval("(" + jsonstr + ")");
    return p;
}
